...
hold on
    ...
    dy = h/(Ny-1);
    L=1;
    x_blasius2=0.0005                   %x value chosen to compare with Blasius results
    x_blasius=0.5
    ...
    u(Ny,:) = 1; % Free outer flow at the top      (UNTIL HERE I AM OK)
...
eta_little=y*sqrt(Re/L/x_blasius2)
...
figure (7)
plot(eta_little,v(:,(Nx-1)/2),'linewidth',2)
title('v velocity profile at x=0.5 solving the boundary layer problem')
xlabel('\eta pour x = 0.0005', 'FontSize', 14);
ylabel(" u", 'FontSize', 14);
grid on;
